...
ZCAN_ZPSCANFD_TCP         = ZCAN_DEVICE_TYPE(78)
ZCAN_ZPSCANFD_USB         = ZCAN_DEVICE_TYPE(79)
...
def canfd_start(zcanlib, device_handle, chn):
    ...
    if ret != ZCAN_STATUS_OK:
        ...
        exit(0)
    ret = zcanlib.SetValue(ip, str(chn) + "/initenal_resistance", "0")
    if ret != ZCAN_STATUS_OK:
...
###SET filter  
    ...
    if ret != ZCAN_STATUS_OK:
        print("startCAN failed!" %(chn_handle))
        exit(0)  
...
def task_trans(can_lib = None, channel = 0, thrad_keyboard = None):
    transmit_num = 2
    ...
    while True:
        ret = can_lib.Transmit(channel, msgs, transmit_num)
        ret = can_lib.TransmitFD(channel, canfd_msgs, transmit_canfd_num)
        time.sleep(1)
        print("Tranmit Num: %d." % ret)
        if thrad_keyboard.is_alive() == False:
            break
...
if __name__ == "__main__":
    ...
    print('Current can device sn is :', ZCAN_USBCANFD_400U)
    handle = zcanlib.OpenDevice( ZCAN_USBCANFD_400U, 0,0) #ZCAN_USBCANFD_400U
    ...
    chn_handle0 = canfd_start(zcanlib, handle, 0)
    chn_handle2 = canfd_start(zcanlib, handle, 2)
    print("channel handle:%d." %(chn_handle0))
    print("channel handle:%d." %(chn_handle2))

    thread=threading.Thread(target=input_thread)
    thread.start()
    # Start to create thread
    mthread = threading.Thread(target=task_trans, args=(zcanlib, 0, thread) ) 
    # 启动刚刚创建的线程 
    mthread .start()
    ...
    #Receive Messages
    while True:
        rcv_num = zcanlib.GetReceiveNum(chn_handle2, ZCAN_TYPE_CAN)
        rcv_canfd_num = zcanlib.GetReceiveNum(chn_handle2, ZCAN_TYPE_CANFD)
        if rcv_num:
            print("Receive CAN message number:%d" % rcv_num)
            rcv_msg, rcv_num = zcanlib.Receive(chn_handle2, rcv_num)
            ...
            rcv_canfd_msgs, rcv_canfd_num = zcanlib.ReceiveFD(chn_handle2, rcv_canfd_num, 1000)
    ...
    #Close CAN 
    ret=zcanlib.ResetCAN(chn_handle0)
    ret=zcanlib.ResetCAN(chn_handle2)
    if ret==1:
